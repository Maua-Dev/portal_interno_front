...
import stylesUpdateUser from './update_user.module.css'
import { UserContext } from '@/contexts/user_provider'
import LinkComponent from '../components/Link'

export default function UpdateUser() {
  const { updateUser, users, getUser, error, setErrorNull } =
        ...
        {users.map((user) => {
          return (
            <>
              <div
                key={user.id}
                className={stylesUpdateUser.container__users__update}
              >
                ...
                <form
                  onSubmit={(event) =>
                    handleUpdateUser(user.id as number, event)
                  }
